...
if [[ -e "$dir/Makefile" ]]; then #check if Makefile exist
	...
	valgrind --leak-check=yes ./$cf >"bash $dir/test.txt" 2>&1 #run and creates valgrid output file
	...
	valgrind --tool=helgrind ./$cf >"bash $dir/test.txt" 2>&1 #run and creates helgrid output file

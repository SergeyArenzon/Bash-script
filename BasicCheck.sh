...
if [[ -e "$dir/Makefile" ]]; then #check if Makefile exist
	...
	valgrind --leak-check=yes ./$cf >"$dir/test.txt" echo 2>&1 #run and creates valgrid output file
	...
	valgrind --tool=helgrind ./$cf >"$dir/test.txt" echo 2>&1 #run and creates helgrid output file
